...
class Tickets_model extends CI_Model{
    ...
    function get_datetickets($dt){
        $this->db->select("support_time, COUNT(ticket_no) as tot_ticket");        
        $this->db->from("supports");
        $this->db->group_by("DATE(support_time)");
        $this->db->where("DATE(support_time)", $dt);
        $query = $this->db->get(); 
        if($query->num_rows() == 0) {
            return FALSE;
        } else {
    ...
    }//End of get_datetickets()
        
    function get_datetypetickets($dt, $typ){
        $this->db->select("support_time, COUNT(ticket_no) as tot_ticket");        
        $this->db->from("supports");
        $this->db->group_by("DATE(support_time)");
        $this->db->where("DATE(support_time)", $dt);
        $this->db->where("support_status", $typ);
        $query = $this->db->get(); 
        if($query->num_rows() == 0) {
            return FALSE;
        } else {
