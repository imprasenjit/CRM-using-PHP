...
class Ticketsopen extends Alom {
    ...
    function index() {
        $this->load->view("ticketsopen_view");
    }//End of index()
    ...
    function getRecords() {
        ...
        $totalData = $this->ticketsopen_model->tot_rows();
        $totalFiltered = $totalData;
        if (empty($this->input->post("search")["value"])) {
            $records = $this->ticketsopen_model->all_rows($limit, $start, $order, $dir);
        } else {
            $search = $this->input->post("search")["value"];
            $records = $this->ticketsopen_model->search_rows($limit, $start, $search, $order, $dir);
            $totalFiltered = $this->ticketsopen_model->tot_search_rows($search);
        }
        $data = array();
        if (!empty($records)) {
    ...
    function getUserRecords() {
        ...
        $order = $columns[$this->input->post("order")[0]["column"]];
        $dir = $this->input->post("order")[0]["dir"];
        $totalData = $this->ticketsopen_model->tot_userrows($uid);
        $totalFiltered = $totalData;
        if (empty($this->input->post("search")["value"])) {
            $records = $this->ticketsopen_model->all_userrows($uid, $limit, $start, $order, $dir);
        } else {
            $search = $this->input->post("search")["value"];
            $records = $this->ticketsopen_model->search_userrows($uid, $limit, $start, $search, $order, $dir);
            $totalFiltered = $this->ticketsopen_model->tot_search_userrows($uid, $search);
        }
        $data = array();
        if (!empty($records)) {
            foreach ($records as $rows) {
