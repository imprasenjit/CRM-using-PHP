...
class Ticketsnew extends Alom {
    ...
    function index() {
        $this->load->view("ticketsnew_view");
    }//End of index()
    ...
    function getRecords() {
        ...
        $totalData = $this->ticketsnew_model->tot_rows();
        $totalFiltered = $totalData;
        if (empty($this->input->post("search")["value"])) {
            $records = $this->ticketsnew_model->all_rows($limit, $start, $order, $dir);
        } else {
            $search = $this->input->post("search")["value"];
            $records = $this->ticketsnew_model->search_rows($limit, $start, $search, $order, $dir);
            $totalFiltered = $this->ticketsnew_model->tot_search_rows($search);
        }
        $data = array();
        if (!empty($records)) {
    ...
    function getUserRecords() {
        ...
        $order = $columns[$this->input->post("order")[0]["column"]];
        $dir = $this->input->post("order")[0]["dir"];
        $totalData = $this->ticketsnew_model->tot_userrows($uid);
        $totalFiltered = $totalData;
        if (empty($this->input->post("search")["value"])) {
            $records = $this->ticketsnew_model->all_userrows($uid, $limit, $start, $order, $dir);
        } else {
            $search = $this->input->post("search")["value"];
            $records = $this->ticketsnew_model->search_userrows($uid, $limit, $start, $search, $order, $dir);
            $totalFiltered = $this->ticketsnew_model->tot_search_userrows($uid, $search);
        }
        $data = array();
        if (!empty($records)) {
            foreach ($records as $rows) {
